...
class FundusSegmentation(Dataset):
    ...
    def __init__(self,
        ...
        self.img_idx_pool = {'DGS':[], 'REF':[], 'RIM':[], 'REF_val':[]}
    ...
    def __len__(self):
        ...
        return max  # 为什么用最大值不用和

    def __getitem__(self, index):
        if self.phase != 'test':
            sample = [] # 每个域抽一个样本(图片,标签,域码,文件名)
            for key in self.image_pool:
                ...
                _img_idx = self.img_idx_pool[key][index]
                anco_sample = {'image': _img, 'label': _target, 'img_name': _img_name, 'dc': domain_code, 'img_idx': _img_idx}
    ...
    def _read_img_into_memory(self):
        ...
        for index in range(img_num):
            ...
            if basename[0:2] in self.flags_DGS:
                Flag = 'DGS'
                DGS_idx += 1
                self.img_idx_pool[Flag].append(DGS_idx)
            elif basename[0] in self.flags_REF:
                Flag = 'REF'
                REF_idx += 1
                self.img_idx_pool[Flag].append(REF_idx)
            elif basename[0] in self.flags_RIM:
                Flag = 'RIM'
                RIM_idx += 1
                self.img_idx_pool[Flag].append(RIM_idx)
            elif basename[0] in self.flags_REF_val:
                Flag = 'REF_val'
                REF_val_idx += 1
                self.img_idx_pool[Flag].append(REF_val_idx)
            else:
