...
def main():
    ...
    parser.add_argument('--model-file', type=str, default='./logs/test/[1]/20260329_155453.669517/checkpoint_1_best.pth.tar', help='Model path')
    ...
    parser.add_argument('--data-dir', default=r'./Datasets', help='data root path')
    ...
    parser.add_argument('--test-prediction-save-path', type=str, default='./results/final/', help='Path root for test image and mask')
    ...
    for batch_idx, (sample) in tqdm.tqdm(enumerate(test_loader),total=len(test_loader),ncols=80, leave=False):
        data = sample['image']  # [12,3,256,256]
        target = sample['label']  # [12,3,800,800]
        img_name = sample['img_name']  # [12]
        if torch.cuda.is_available():
            data, target = data.to(device), target.to(device)
        data, target = Variable(data), Variable(target)

        prediction = model(data, mode='val') # [12,3,256,256]
        ...
        for i in range(prediction.shape[0]):
            prediction_post = postprocessing(prediction[i], dataset=args.dataset)    # [2,800,800]
